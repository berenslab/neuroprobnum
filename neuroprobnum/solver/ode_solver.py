...
class ProbSolver:
    ...
    def __init__(
            ...
            pert_method=None, pert_param=1.0,
            n_events=0, prestepfun=None, poststepfun=None, presolvefun=None,
        ...
        pert_method : str or None
            ...
            Chose either "conrad", "abdulle_un" or "abdulle_ln" for perturbation.
        pert_param : float
        ...
        # Perturbation.
        self.pert_param_order = None
        if pert_method is None:
            self.pert_method = None
        else:
            self.pert_method = pert_method.lower()
            assert self.pert_method in ['conrad', 'abdulle_un', 'abdulle_ln'], self.pert_method
            if self.pert_method in ['abdulle_un', 'abdulle_ln']:
                self.pert_param_order = self.order_B
            self.check_pert_params()
        self.pert_param = float(pert_param) if self.pert_method is not None else None
        ...
        # For perturbation
        self._pert_bound = None
        self._pert_mu = None
        self._pert_sigma = None

        # For debugging
        self.last_eval_odefun_t = None
        self.last_eval_odefun_y = None
        self.last_eval_odefun_ydot = None

        self.reset()

    def check_pert_params(self):
        """Sanity check perturbation parameters"""
        if self.pert_method == 'abdulle_un':  # Sanity checks:
            assert self.pert_param_order >= 0.5, self.pert_param_order
            max_h_unpert = self.max_step if self.adaptive else self.h0
            max_pert = self.pert_param * max_h_unpert ** (self.pert_param_order + 0.5)
            assert max_pert < max_h_unpert, \
                f'Neg. hs possible. Ensure pp={self.pert_param} < {max_h_unpert ** (0.5 - self.pert_param_order)}'
    ...
    def reset(self):
        ...
        if not self.adaptive:
            self._update_pert_distributions(self.h0)

    def eval_odefun(self, t, y, count=True):
        """Evaluate ODE function"""

        if self.nargs_odefun == 2:
            ydot = self.odefun(t, y)
        elif self.nargs_odefun == 1:
            ydot = self.odefun(y)
        else:
            ydot = self.odefun(t, y, self)
    ...
    def solve_sample(
        ...
        self.reset()
        start_time = time()
        if self.presolvefun is not None:
            self.presolvefun(solver=self)
        post_step_ydot_new = 'ydots' in return_vars  # Update ydot after step?

        if show_progress:
            pbar = tqdm(total=tmax - self.t0, bar_format=TQDM_BARFORMAT)
        ...
        while self.t < tmax:
            ...
            if not self.step(step_tmax):
                break  # Break at non-successfull step
            if self.poststepfun is not None:
                self.poststepfun(solver=self)
            ...
            # Prepare next step.
            # Prevent round errors if possible
            self.t = self.t_new if self.adaptive or not save_step else self.solution.get_t()
    ...
    def step(self, step_tmax):
        ...
        if self.adaptive:
            step_success = self.try_step_adaptive(
                step_tmax=step_tmax, save_tried_steps=self.solution.store_failed_steps)
        else:
        ...
        if step_success:
            ...
            elif self.pert_method in ['abdulle_un', 'abdulle_ln']:
                self.ydot_new = None
    ...
    def try_step(self):
        ...
        elif self.pert_method == 'abdulle_un':
            if self.adaptive:
                self._update_pert_distributions(self.h)
    ...
    def _update_pert_distributions(self, h):
        if self.pert_method == 'abdulle_un':
            self._pert_bound = self.pert_param * h ** (self.pert_param_order + 0.5)

        elif self.pert_method == 'abdulle_ln':
            msquared = h ** 2
            phi = np.sqrt(self.pert_param ** 2 * h ** (2 * self.pert_param_order + 1) + msquared)
    ...
    def try_step_adaptive(self, step_tmax, save_tried_steps):
        ...
        while not self.step_accepted:
            ...
            # Save step.
            if save_tried_steps:
                self.solution.save_tried_step(h=self.h, success=self.step_accepted)

            # Adapt step size.
            if not self.step_accepted:
                if self.DEBUG:
                    assert self.f_h < 1.0, self.f_h
                self.step_any_rejected = False

            if self.step_accepted and self.step_any_rejected:
                self.f_h = np.min([self.f_h, 1.])
...
class ExplicitInt(ProbSolver):
    ...
    def dense_eval_at_t(self, t_eval, yidxs=None):
        ...
        if self.pert_method == 'conrad':
            ...
            _weighted_perturbation = _perturbation * _dt if isinstance(t_eval, (float, int)) else np.outer(
                _perturbation, _dt)
...
class ImplicitInt(ProbSolver):
    ...
    def __init__(self, adaptive=False, **kwargs):
        ...
        if adaptive:
            raise NotImplementedError('Step size adaptation not implemented')
...
class ExponentialInt(ProbSolver):
