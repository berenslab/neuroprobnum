...
cdef class CHHNeuron:
    ...
    cdef void update_ydot(self):
        ...
        self.ydot[iy_v] = 0.0 if self.v_clamped else (self.yinf[iy_v] - self.y[iy_v]) * self.yf[
            iy_v]  # [mV * kHz = mV / ms]
        self.ydot[iy_n] = (self.yinf[iy_n] - self.y[iy_n]) * self.yf[iy_n]  # [kHz = 1 / ms]
        self.ydot[iy_m] = (self.yinf[iy_m] - self.y[iy_m]) * self.yf[iy_m]  # [kHz = 1 / ms]
        self.ydot[iy_h] = (self.yinf[iy_h] - self.y[iy_h]) * self.yf[iy_h]  # [kHz = 1 / ms]
    ...
    def eval_ydot_us(self, t, y):
        """Set t and y, update ydot, return ydot"""
        t = t * 1e-3  # us to ms
        ydot = self.eval_ydot(t, y)
        return ydot * 1e-3  # 1/us to 1/ms

    def eval_yinf_and_yf_us(self, t, y):
        """Set t and y, update yinf and yf, return yinf and yf"""
        t = t * 1e-3
        yinf, yf = self.eval_yinf_and_yf(t, y)
        return yinf, yf * 1e-3

    def eval_ydot_s(self, t, y):
        """Set t and y, update ydot, return ydot"""
        t = t * 1e3  # s to ms
        ydot = self.eval_ydot(t, y)
        return ydot * 1e3  # 1/ms to 1/s

    def eval_yinf_and_yf_s(self, t, y):
        """Set t and y, update yinf and yf, return yinf and yf"""
        t = t * 1e3
        yinf, yf = self.eval_yinf_and_yf(t, y)
        return yinf, yf * 1e3

    def eval_ydot_doublespeed(self, t, y):
        """Set t and y, update ydot, return ydot"""
        ydot = self.eval_ydot(t, y)
        return ydot * 2.

    def eval_yinf_and_yf_doublespeed(self, t, y):
        """Set t and y, update yinf and yf, return yinf and yf"""
        yinf, yf = self.eval_yinf_and_yf(t, y)
        return yinf, yf * 2.
