...
class data_generator():
    ...
    def gen_sol(
            ...
            pert_param='auto', n_samples=None, plot=False
        ):
        """Generate samples."""
    ...
    def get_data_folder_and_filename(self, method, adaptive, step_param, pert_method, pert_param='auto'):
        """Create data folder."""
        folder = f'{self.base_folder}'
        if self.subfoldername is not None: folder += f'/{self.subfoldername}'
        filename = f'{method}_'
    ...
    def load_data_and_check(
            self, method, adaptive, step_param, pert_method,
            pert_param='auto', filename=None, stim=None,
        ):
        ...
        if self.gen_det_sols:
            ...
            elif ((not adaptive) or (self.t_eval_adaptive is not None)) and (not math_utils.arrs_are_equal(data.det_ts, data.ts)):
