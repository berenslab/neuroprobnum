...
import plot_utils
from math_utils import intpol_ax0, arrs_are_equal
...
class ode_solutions(ode_solution_base):
    ...
    def __init__(self, solutions, run_time=None):
        ...
        if not self.success:
            print('Exited with error in {:.2f} samples.'.format((self.n_samples-np.sum(self.success_list))/self.n_samples))
    ...
    def _plot_data(axs, ts, data, y_idxs, summary_plot=False, max_samples=30):
        ...
        if isinstance(data, list):
            if summary_plot: print('Convert to array by interpolation for summary plot')
            plot_data_list()
        else:
            if summary_plot: plot_data_array_summary()
            else: plot_data_array()
