...
class neuron(chh.neuron):
    ...
    def plot_alpha_beta(self, vs=np.linspace(-100, 50, 201)):
        """Plot alpha and beta"""
        fig, axs = plt.subplots(2, 3, figsize=(8, 3), sharex=True, sharey=True,
    ...
    ### Subunits parameters, array functions ###
    @staticmethod
    def compute_alpha_x(x, vs):
        vs = np.atleast_1d(vs)
        if x == 'n': return np.array([chh.compute_alpha_n(v) for v in vs])
        if x == 'm': return np.array([chh.compute_alpha_m(v) for v in vs])
        if x == 'h': return np.array([chh.compute_alpha_h(v) for v in vs])
        
    @staticmethod
    def compute_beta_x(x, vs):
        vs = np.atleast_1d(vs)
        if x == 'n': return np.array([chh.compute_beta_n(v) for v in vs])
        if x == 'm': return np.array([chh.compute_beta_m(v) for v in vs])
        if x == 'h': return np.array([chh.compute_beta_h(v) for v in vs])
